...
All 120 automated tests pass after the operator usability update. They cover reservations, per-cell actuals, duplicate/lost-response retry, first-arrival locking, shared cells, partial redistribution, supervisor shortages, stale plans, restart, account revocation, unit-aware/net corrections, transaction rollback, ledger reporting, QR encoding/decoding, hardware command guards, progressive work screens and navigation focus/disclosure state.
...
## Operator usability update — 25 September 2026

Operators sign in to My work, with My work / Pick / Put as the primary navigation. More tools retains every other permitted screen; admins retain their full navigation. Section links and shortcut disclosure controls have separate click targets. The responsive menu uses one breakpoint, supports Escape, restores focus across resizing, and leaves navigation visible if its JavaScript fails to load.

Normal planning asks for product and quantity; preferred location is optional disclosure. Ready locations show arrival first, followed by an explicit blank actual quantity after starting. Zero and partial actuals remain supported. Notes, manual-work recovery, cancellation/replanning and corrections stay available through labelled disclosures. Offline/pending/do-not-repeat feedback remains visible. Active or review tasks precede collapsed completed history, and periodic refresh preserves open disclosures and entered values.

The isolated simulator passed browser checks at approximately 390 px phone and 1430 px desktop widths: operator/admin landing and navigation, separate link/shortcut behavior, menu toggling, Escape, desktop-to-mobile focus, report-modal closure followed by navigation, blank-actual rejection, wrong cell rejection, partial pick, zero and positive put, manual report and admin reconciliation. A saved offline report survived reload and was delivered under its original ID after simulator restart. Independent browser checks also verified disclosure state through timed refresh. The public offline asset cache advances to version 5 after successful installation; device report storage and server permission/ledger rules are unchanged. Real devices and warehouse hardware still require the physical acceptance described above.
