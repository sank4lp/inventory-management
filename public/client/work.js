...
async function refresh(){
 ...
 if(snapshot&&(fresh.user.id!==snapshot.user.id||fresh.site!==snapshot.site))throw new Error('The signed-in account or warehouse changed. Reopen My work; saved reports stay with their original account and warehouse.');
...
function performerField(selected='unknown') {
 ...
 return `<label>Verified performer<select name="performerId"><option value="unknown" ${selected==null||selected==='unknown'?'selected':''}>Unknown / unverified</option>${options(snapshot.performers||[],'id',u=>`${u.name} · ${u.username}${u.status==='active'?'':' (inactive)'}`,selected)}</select></label><p class="work-help">Who physically moved the stock. The authenticated reporter and verifying supervisor remain recorded separately.</p>`;
}
function accountingFields(r){
 const a=r.accounting;if(!a||a.available&&a.unit===r.unit)return '';
 return `<p class="work-callout">Original report: ${esc(r.quantity)} ${esc(r.unit)}. ${a.available?`Recorded history: multiply by ${esc(a.factor)}; this report accounts for ${esc(a.quantity)} ${esc(a.unit)}. If you correct the original actual, verify the corresponding converted quantity.`:`No reliable conversion available: ${esc(a.reason)} Establish the current-unit actual from physical evidence; do not guess a factor.`}</p>${hidden('accountingUnit',a.unit)}${qty('Verified accounting quantity ('+a.unit+')').replace('name="quantity"','name="accountingQuantity"')}${!a.available?input('conversionProvenance','Evidence for current-unit quantity','text','required placeholder="Physical count / pack size checked / dated receiving record"'):''}`;
}
...
function lineCard(l){
 ...
 const working=l.execution_state==='working';
 let actions='';
 if(active&&l.canAct!==false){
  const arrival=form('acquire',allocationFields(l)+`<div class="work-arrival">${input('location','Cell code','text','required autocomplete="off" placeholder="Scan or type the printed code"')}<button type="button" class="secondary" data-scan>Scan QR</button></div><button>${working?'Resume this location':'Start at this location'}</button>`);
  if(online&&!waiting.length&&!saved) actions+=working?disclosure('resume-'+l.id,'Verify location again',arrival):`<p class="work-eyebrow">1 · Verify your location</p>${arrival}`;
  const manual=!online||!working;
  const report=form('report',allocationFields(l)+qty('Actual quantity ('+l.unit_of_measure+')')+performerField(l.created_by)+disclosure('note-'+l.id,'Add a note or report a problem',input('reason','Note (optional)')+(manual?'':`<label class="work-check"><input type="checkbox" name="manual">Work performed manually / connection interrupted</label>`))+ (manual?hidden('manual','true'):'')+`<p class="work-help">Enter what you actually ${l.type==='pick'?'picked':'put'}. Use 0 if nothing moved. Each location is recorded separately.</p><button ${saved?'disabled':''}>${saved?'Report saved — awaiting receipt':'Confirm actual quantity'}</button>`);
  actions+=working&&online&&!waiting.length?`<p class="work-eyebrow">2 · Record what you moved</p>${report}`:disclosure('manual-'+l.id,'Already moved stock or connection problem?',`<p class="work-callout warning">Report only work that already happened. The warehouse may need supervisor verification; do not repeat the movement.</p>${report}`,!online);
  if(l.execution_state==='ready'&&online&&!waiting.length&&!saved){
   const cancel=form('cancel',allocationFields(l)+'<label class="work-check"><input type="checkbox" required>I have not moved any stock at this location.</label><button class="secondary">Cancel this location</button>');
   const replan=l.type==='put'?form('replan',hidden('lineId',l.id)+hidden('revision',l.revision)+`<label>Replacement location<select name="cellId">${options(snapshot.cells,'id',c=>c.logical_code,l.cell_id)}</select></label>`+qty('Replacement planned quantity')+'<label class="work-check"><input type="checkbox" required>No stock has been moved for the original plan.</label><button class="secondary">Save replacement plan</button>'):'';
   actions+=disclosure('change-'+l.id,l.type==='put'?'Change or cancel this location':'Cancel this location',cancel+replan);
  }
 }else if(l.execution_state==='settled'&&l.canAct!==false)actions+=disclosure('correct-'+l.id,'Correct this earlier record','<p>Correct the earlier actual only. A new return or pick is a separate movement.</p>'+form('correct',allocationFields(l)+qty('Correct actual quantity')+input('verification','Correction reason','text','required')+'<button>Save correction</button>'));
 return `<article class="allocation"><div class="work-card-heading"><div><span class="work-eyebrow">${esc(l.type)} · ${esc(l.guidance_mode==='shared'?'Shared location':'Your location')}</span><h2 class="cell-code">${esc(l.logical_code)}</h2></div>${badge(status(l.execution_state),l.execution_state==='settled'?'good':'')}</div><div class="work-quantity"><strong>${esc(l.execution_state==='settled'?l.actual_quantity:l.planned_quantity)}</strong><span>${esc(l.unit_of_measure)}<small>${l.execution_state==='settled'?'actual recorded':l.execution_state==='superseded'?'earlier plan — do not execute':l.execution_state==='cancelled'?'cancelled plan — do not execute':'planned quantity'}</small></span></div><p class="work-product">${esc(l.product_name)} <span>${esc(l.sku)}</span></p>${l.guidance_mode==='shared'?'<p class="work-callout">The neutral light locates this cell. Follow your action and quantity on this screen.</p>':''}${waiting.map(r=>`<p class="work-callout warning">Needs review: ${esc(r.reason)}</p>`).join('')}${saved?'<p class="work-callout">Your report is saved on this device. Do not repeat the movement.</p>':''}${l.canAct===false?'<p class="work-callout">View only. This work belongs to another operator.</p>':''}${actions}</article>`;
}
...
function manualPage(direction=null){return `<section class="work-intro"><span class="work-eyebrow">Capture physical work</span><h2>${direction?'Offline '+esc(direction)+' · record physical work':'Record what already happened'}</h2>${!online?'<p>Continue physical work using the warehouse manual procedure. Enter the actual after moving stock. This provisional report stays on this device until the warehouse receives it for supervisor verification; it does not reserve stock or grant an exclusive turn.</p>':''}<p>Keep the original movement reference across phone, paper, and shared terminal. A supervisor verifies the report; submitting it does not activate lights.</p></section><section class="work-panel narrow">${form('manual',`<label>Movement<select name="direction"><option value="pick" ${direction==='pick'?'selected':''}>Picked / removed</option><option value="put" ${direction==='put'?'selected':''}>Put / returned</option><option value="count">Count observation</option></select></label><label>Product<select name="productId" required>${options(snapshot.products,'id',p=>`${p.name} · ${p.unit_of_measure}`)}</select></label><label>Location<select name="cellId" required>${options(snapshot.cells,'id',c=>c.logical_code)}</select></label>`+qty()+input('unit','Reported unit (leave blank for current product unit)','text','placeholder="Use original unit for an older movement"')+performerField()+input('origin','Original reference (reuse the same reference on paper)','text',`required maxlength="180" ${!online?'value="'+esc(provisionalReference())+'"':''}`)+input('occurredAt','When it happened (optional)','datetime-local')+input('reason','What happened / uncertainty','text','required')+`<p class="work-help">Use a count as an observation during ongoing work. It will not replace the stock balance. Operators report their own work. A supervisor can attribute recovery reports to an existing account or leave the performer unknown.</p><button>Save completed movement report</button>`)}</section>`;}
...
function taskCards(tasks){return tasks.map(t=>`<a class="work-task" href="/tasks/${t.id}" data-local-task="${t.id}"><div class="work-card-heading"><span class="work-eyebrow">${esc(t.type)} · #${t.id}</span>${badge(t.attention?'Needs review':status(t.status),t.attention?'warning':t.status==='completed'?'good':'')}</div><h3>${esc(t.summary)}</h3><p>${t.lines.filter(l=>['settled','cancelled'].includes(l.execution_state)).length} of ${t.lines.filter(l=>l.execution_state!=='superseded').length} locations closed${snapshot.user.role==='admin'?' · '+esc(t.created_by_name):''}</p><span class="work-link">Open task →</span></a>`).join('');}
function home(){
 const closed=t=>!t.attention&&['completed','cancelled'].includes(t.status);
 const active=snapshot.tasks.filter(t=>!closed(t)),completed=snapshot.tasks.filter(closed);
 return `<section class="work-intro"><span class="work-eyebrow">My work</span><h2>What’s your next move?</h2><p>Start a new task or continue where you left off.</p><div class="work-actions"><a class="work-primary" href="/pick">Pick stock</a><a class="work-secondary" href="/put">Put stock</a></div></section><h3 class="work-section-title">In progress${active.length?' · '+active.length:''}</h3><div class="work-grid">${taskCards(active)||'<section class="work-empty"><h3>You’re ready for a new task</h3><p>Your active picks and puts will appear here.</p></section>'}</div>${completed.length?disclosure('completed-work','Completed work · '+completed.length,`<div class="work-grid">${taskCards(completed)}</div>`,false,'work-archive'):''}`;
}
...
function render(){
 ...
 root.innerHTML=`<div class="work-toolbar"><div class="work-tools">${!online?'<a href="/work">My work</a><a href="/pick">Pick</a><a href="/put">Put</a>':''}${disclosure('work-tools','Work tools',`<div class="work-tools-links"><a href="/record-movement">Record completed movement</a><a href="/movement-history">Movement history</a><a href="/labels">Location labels</a>${snapshot.user.role==='admin'?'<a href="/pending-confirmations">Pending confirmations</a>':''}</div>`,false,'work-tools-details')}</div><div class="work-connection">${badge(online?'Connected to warehouse':'Offline · saved work',online?'good':'warning')}<small>Updated ${esc(new Date(snapshot.generatedAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}))}</small></div></div><div id="work-notice" role="status" aria-live="polite">${notice?`<p class="work-callout">${esc(notice)}</p>`:''}</div>${!online?'<p class="work-callout warning">Offline: saved work and physical reports stay on this device. No new reservation or location turn is granted. Follow the warehouse manual procedure; use paper if needed.</p>':''}${queued.length?`<section class="work-queue"><h3>${queued.length} report${queued.length===1?'':'s'} to follow up</h3>${queued.map(o=>`<p>${badge(status(o.state),o.state==='review'?'warning':'')} ${esc(o.message||'Do not repeat the movement.')} <small>${esc(o.id)}</small></p>`).join('')}<button type="button" class="secondary" data-retry>Send saved reports / refresh</button></section>`:''}${body}<footer class="work-footer">${disclosure('device-recovery','Saved reports & device help','<p>Saved work stays on this device. Use your own device account. Send all reports before handing it over.</p><div class="work-tools-links"><button type="button" class="text-button" data-export>Export saved reports for reconciliation</button><button type="button" class="text-button" data-forget>Clear local data (only after reports are received)</button></div>')}</footer>`;
...
// Reveal invalid controls inside disclosures before the browser tries to focus them.
...
root.addEventListener('submit',async e=>{
 ...
 try{
  ...
  for(const n of ['manual','keepOpen','dismissDuplicate','closeAllocation'])if(n in values)values[n]=values[n]==='on'||values[n]==='true';
  ...
  if(action!=='acquire'){
   ...
   await store('outbox','readwrite',s=>s.put(entry));if(action==='manual')provisionalReference(true);outbox=await all('outbox');notice='Saved on this device. Awaiting warehouse receipt.';render();await sync();const received=outbox.find(o=>o.id===id);if(action==='create'&&received?.result?.taskId){location.href='/tasks/'+received.result.taskId;return;}render();
  }else{
...
const canRefresh=()=>!dirty&&!root.contains(document.activeElement)&&!cameraStream?.active;
async function backgroundRefresh(){if(!canRefresh())return;await sync();if(canRefresh())render();}
window.addEventListener('online',backgroundRefresh);
document.addEventListener('visibilitychange',async()=>{if(document.visibilityState==='visible')await backgroundRefresh();});
try{
 db=await openDB();
...
setInterval(()=>{if(document.visibilityState==='visible')backgroundRefresh();},30000);
