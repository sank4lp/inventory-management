...
function lineCard(l){
 ...
 if(active&&l.canAct!==false){
  ...
  actions+=form('report',allocationFields(l)+qty()+performerField(l.created_by)+input('reason','Note (optional)')+`<label class="work-check"><input type="checkbox" name="manual" ${!online||l.execution_state!=='working'?'checked':''}>Work performed manually / connection interrupted</label><p class="work-help">Enter what you actually ${l.type==='pick'?'picked':'put'}, even if it differs from the plan. Zero closes this allocation with no movement.</p><button ${saved?'disabled':''}>${saved?'Report saved — awaiting receipt':'Confirm actual quantity'}</button>`);
...
function render(){
 ...
 root.innerHTML=`<div class="work-toolbar"><nav aria-label="Warehouse work"><a href="/work">My work</a><a href="/record-movement">Record movement</a><a href="/movement-history">History</a>${snapshot.user.role==='admin'?'<a href="/pending-confirmations">Pending confirmations</a>':''}</nav>${badge(online?'Connected to warehouse':'Offline · saved work',online?'good':'warning')}</div><p class="work-help">${esc(snapshot.user.name)} · Saved ${esc(new Date(snapshot.generatedAt).toLocaleString())}</p><div id="work-notice" role="status" aria-live="polite">${notice?`<p class="work-callout">${esc(notice)}</p>`:''}</div>${!online?'<p class="work-callout warning">Continue cached work or start physical work using the warehouse manual procedure. Use Pick, Put, or Record movement to save provisional actual reports on this device. No new stock reservation or exclusive turn is granted offline. Use paper if no usable device is available.</p>':''}${queued.length?`<section class="work-queue"><h3>${queued.length} report${queued.length===1?'':'s'} to follow up</h3>${queued.map(o=>`<p>${badge(status(o.state),o.state==='review'?'warning':'')} ${esc(o.message||'Do not repeat the movement.')} <small>${esc(o.id)}</small></p>`).join('')}<button type="button" class="secondary" data-retry>Send saved reports / refresh</button></section>`:''}${body}<footer class="work-footer"><a href="/labels">Print location labels</a><a href="/">All warehouse functions</a><span>Saved work stays on this device. Use your own device account; clear local data before handing it over.</span><button type="button" class="text-button" data-export>Export saved reports for reconciliation</button><button type="button" class="text-button" data-forget>Clear local data (only after reports are received)</button></footer>`;
}
let syncing=false;
async function sync(){
